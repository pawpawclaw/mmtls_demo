...
class ZStream {
    ...
    func write(_ bytes : [UInt8], flush: Bool) -> (bytes: [UInt8], err: NSError?) {
        ...
        if !initd {
            if deflater {
                if init2 {
                    res = deflateInit2_(&strm, level, Z_DEFLATED, MAX_WBITS + 16, 8, 0, ZLIB_VERSION, CInt(MemoryLayout<z_stream>.size))
                } else {
                    res = deflateInit_(&strm, level, ZLIB_VERSION, CInt(MemoryLayout<z_stream>.size))
                }
            } else {
                if init2 {
                    res = inflateInit2_(&strm, MAX_WBITS + 16, ZLIB_VERSION, CInt(MemoryLayout<z_stream>.size))
                } else {
                    res = inflateInit_(&strm, ZLIB_VERSION, CInt(MemoryLayout<z_stream>.size))
                }
            }
            if res != 0{
